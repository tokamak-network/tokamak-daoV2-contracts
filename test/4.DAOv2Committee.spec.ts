...
describe('DAOv2Committee', () => {
    ...
    describe("#0. setting DAOProxy and daov2Committe", () => {
        ...
        it("DAOProxy upgradeTo ProxyV2", async () => {
            ...
            await DAOProxy.connect(DAOOwner).upgradeTo(DAOProxyV2.address);
            expect(await DAOProxy.implementation()).to.be.eq(DAOProxyV2.address);
        })

        it("DAOProxy connect DAOProxyV2", async () => {
            DAOProxyDAOProxyV2 = await ethers.getContractAt(DAOCommitteProxyV2_ABI.abi, daoCommitteProxyAddress, deployer); 
        })

        it("check storage same", async () => {
            let ton1 = await DAOProxy.connect(DAOOwner).ton()
            let ton2 = await DAOProxyDAOProxyV2.connect(DAOOwner).ton()
            console.log(ton1)
            console.log(ton2)
        ...
        it("DAOProxy setAliveImplementation2 daoCommitteeV2", async () => {
        ...
        it("DAOProxyV2 deployer don't setImplementation2", async () => {
            await expect(
                DAOProxyDAOProxyV2.connect(mainnetDeployer).setImplementation2(deployed.daov2committeeV2.address, 0, true
            )).to.be.revertedWith("DAOCommitteeProxyV2: msg.sender is not an admin");
        })

        it("DAOProxy setImplementation2 daoCommitteeV2", async () => {
            await DAOProxyDAOProxyV2.connect(daoAdmin).setImplementation2(deployed.daov2committeeV2.address, 0, true)
        })
